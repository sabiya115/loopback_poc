...
export function mongooseCorePlugin(schema: mongoose.Schema) {
  const ensureIndex = (that:any) => {
    return that
      .listIndexes()
      .then((idx: any) => {
        return new Promise((resolve, reject) => {
          if (idx.length !== that.schema._indexes.length + 1)
            return that.ensureIndexes({}, (err: any) => {
              if (err) reject(err);
              Promise.resolve();
            });
          Promise.resolve();
  ...
  */
   ...
   */
  /* istanbul ignore next */
  function transformError(err: {isRAWError: any; errors: any;}): BuiltError {
    // d.error(err);
   ...
   */
  /* istanbul ignore next */
  function handleMongoError(error: {name: string; code: any; message: {match: (arg0: RegExp) => string[];}; stack: any;}, rawError: BuiltError): BuiltError {
...
*/
  schema.statics._find = function (func) {
    ...
    return function () {
      ...
      scopes.forEach(function (scope: {query: () => any;}) {
        _lodash.extend(
          query,
  ...
  */
  ...
  function markFieldsModified(params: any) {
    function mergeCustomize(source: any, param: any) {
...
export function pickNested(
  ...
  function pick(destination: {[x: string]: any;}, source: {[x: string]: any;}, key: string) {
    if (key in source) destination[key] = source[key];
  }

  function separate(keys: any[]) {
    return {
      nested: keys.filter(function (key: (string | undefined)[]) {
        return key.indexOf(delim) >= 0;
      }),
      bare: keys.filter(function (key: (string | undefined)[]) {
        return key.indexOf(delim) < 0;
      }),
    };
  }

  function first(key: {split: (arg0: string | undefined) => any[];}) {
    return key.split(delim)[0];
  }

  function rest(key: {split: (arg0: string | undefined) => any[];}) {
    return key.split(delim).splice(1).join(delim);
  }

  function groupKeys(keys: any) {
    return module.exports.groupBy(keys, function (key: any) {
      return first(key);
    });
  }

  function _pickNested(destination: Record<string, any>, source: Record<string, any>, keys: string[]) {
    keys = separate(keys);

    keys.bare.forEach(function (key: any) {
    ...
    diffs.forEach(function (key: string | number) {
        ...
        if (!module.exports.isArray(source[key])) {
          if (module.exports.isObject(source[key])) {
            _pickNested(
              destination[key],
              source[key],
              grouped[key].map(function (k: any) {
                return rest(k);
              }),
            );
          }
